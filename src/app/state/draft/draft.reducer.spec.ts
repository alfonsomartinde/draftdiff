...
describe('draftReducer', () => {
  it('ready marks the given side as ready', () => {
    ...
    const next = draftReducer(s, DraftActions['draft/ready']({ roomId: 'r', side: 'blue' }));
  ...
  it('select sets championId on current step when matches side/type', () => {
    ...
    s = draftReducer(s, DraftActions['draft/select']({ roomId: 'r', side: 'blue', action: 'ban', championId: 12 }));
  ...
  it('confirm advances to next step and sets pending flags', () => {
    let s = createInitialDraftState();
    s = draftReducer(s, DraftActions['draft/select']({ roomId: 'r', side: 'blue', action: 'ban', championId: 12 }));
    s = draftReducer(s, DraftActions['draft/confirm']({ roomId: 'r', side: 'blue', action: 'ban' }));
  ...
  it('confirm on last step sets isFinished=true and countdown=0', () => {
    ...
    s = draftReducer(s, DraftActions['draft/confirm']({ roomId: 'r', side: last.side, action: last.type }));
  ...
  it('tick applies countdown from newState', () => {
    ...
    s = draftReducer(s, DraftActions['draft/tick']({ newState: with10 }));
    expect(s.countdown).toBe(10);
  });

  it('set-team-name updates given team name', () => {
    let s = createInitialDraftState();
    s = draftReducer(s, DraftActions['draft/set-team-name']({ roomId: 'r', side: 'red', name: 'Team R' }));
